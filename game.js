...
const game = {
  ...
  determineInitialTurn() {
    ...
    if (this.roundCount % 2 !== 0) {
      this.turn = "x";
      turnIndicator.src = "icon-x-silver.svg";
    } else {
      this.turn = "o";
      turnIndicator.src = "icon-o-silver.svg";
    }
...
function cellClickHandler(el, isAICaller) {
  ...
  if (isAICaller || !game.isComputerMoving) {
    ...
    if (!game.board[xCoord][yCoord] && !game.roundWinner) {
      ...
      if (game.turn === "x") {
        el.innerHTML = "<img src='icon-x.svg' alt='x' />";
        ...
        el.innerHTML = "<img src='icon-o.svg' alt='o' />";
        // game.turn = "x";
        game.toggleTurn();
...
function cellHoverEffect(el) {
  ...
  if (
    ...
    if (game.turn === "o") {
      el.innerHTML = `<img src='icon-o-outline.svg' alt='o' />`;
    } else {
      el.innerHTML = `<img src='icon-x-outline.svg' alt='x'/>`;
    }
...
function generateModal() {
  ...
  if (game.roundWinner === "tie") {
    ...
    image.src = `icon-${game.roundWinner}.svg`;
    if (!game.isHumanOpponent) {
      //if against computer
